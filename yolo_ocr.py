...
model = YOLO("license_plate_best.pt")                #fine tuned model
...
def correct_plate_format(ocr_text):
    mapping_num_to_alpha = {"0":"1", "1":"I", "5":"S", "8":"B"}
...
#Preprocessing the license plate b4 ocr
def recognize_plate(plate_crop):
    ...
    try:
        ocr_result = reader.readtext(
            plate_resized, detail=0, allowlist="ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789"
        )
        if len(ocr_result) >0:
...
while cap.isOpened():
    ...
    out.write(frame)
    display_frame = cv2.resize(frame, None, fx=0.5, fy=0.5)  # Or fx=0.6 depending on screen
